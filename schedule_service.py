...
from api_client import get_outbound_orders
...
def get_orders_for_scheduling():
    ...
    try:
        ...
        for order in outbound_orders:
            ...
            if (picking_type in ['CASE_PICK'] and pallet_qty == 0):
                cases_to_pick += order_qty
            
            elif (picking_type in ['PIECE_PICK'] and pallet_qty == 0):
                cases_to_pick += round(order_qty / 100)
        
        # Calculate picked pallets
        total_picked_pallets = sum(order.get('pallet_qty', 0) for order in outbound_orders)
        ...
        forecast_data = {
            ...
            'picked_outbound_orders': outbound_orders,
